...
from core.search.helpers import HtmlParser, DateParser, TextProcessor, UrlUtils
from core.search.constants import SEARCH_TOPICS, MAX_CACHE_SIZE, SEARCH_TOPICS as ST
...
class SearchEngine:
    ...
    async def _search_duckduckgo_records(
        ...
        try:
            ...
            def _do_search(region: str):
                with DDGS() as ddgs:
                    kw = dict(max_results=5, region=region)
                    if timelimit:
                        kw["timelimit"] = timelimit
                    return list(ddgs.text(query, **kw))

            region_tasks = [asyncio.to_thread(_do_search, r) for r in regions]
            region_results = await asyncio.gather(*region_tasks, return_exceptions=True)

            items: List[Dict[str, str]] = []
            seen: Set[str] = set()
            for region, results in zip(regions, region_results):
            ...
            self.logger.info(
                f"[Search Primary] Query: '{query}' regions={regions} -> {len(items)} items in {latency_ms}ms"
            )
            return items
    ...
    def _count_quality_sources(self, records: List[Dict[str, str]], topic: str, query: str = "") -> int:
        ...
        for rec in records:
            ...
            if self._is_quality_record(topic, query, rec):
                quality_domains.add(UrlUtils.organization_domain(domain) or domain)
    ...
    def _dynamic_reputation_score(self, topic: str, query: str, record: Dict[str, str]) -> float:
        ...
        score = 0.0
        score += TextProcessor.query_coverage_score(query, f"{title} {snippet}")
        if evidence:
            score += 0.75 * TextProcessor.query_coverage_score(query, f"{title} {evidence}")

        overlap_snippet = TextProcessor.query_overlap_count(query, f"{title} {snippet}")
        ...
        if evidence:
            overlap_evidence = TextProcessor.query_overlap_count(query, f"{title} {evidence}")
        ...
        if any(token for token in re.findall(r"\w+", query.lower()) if len(token) > 3 and token in title.lower()):
            score += 0.35
    ...
    def _lightweight_rerank(self, topic: str, query: str, records: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        for rec in records:
            ...
            tokens = set(TextProcessor.normalize_query_tokens(text))
        ...
        for idx, rec in enumerate(records):
            ...
            if is_time_sensitive:
                pub_date = DateParser.extract_date(rec.get("snippet", "")[:40])
                if not pub_date:
                    pub_date = DateParser.extract_date(rec.get("title", "")[:40])
                decay_penalty = self._calculate_time_decay_penalty(topic, pub_date, is_time_sensitive)

            d_penalty = 0.0
            domain = (rec.get("domain") or "").strip().lower()
            org_domain = UrlUtils.organization_domain(domain) or domain
            if org_domain:
    ...
    def _format_final_search_result(self, topic: str, query: str, ranked_records: List[Dict[str, str]], required_sources: int) -> str:
        ...
        header = (
            ...
            f"Search policy: time-sensitive={self._is_time_sensitive_query(query)}\n"
            f"Selected/evidence results: {len(top)}\n"
        )
        parts = [header]
        ...
        for idx, rec in enumerate(top, start=1):
            ...
            url = (rec.get("url") or "#").strip()
            source = rec.get("domain") or UrlUtils.normalize_domain(url)
            ...
            if evidence:
                block += f"      NỘI DUNG: {evidence[:2500]}"
            elif snippet:
                block += f"      SNIPPET: {snippet[:800]}"
            parts.append(block)

        return "\n\n".join(parts)

    def _format_search_ranked_debug(self, records: List[Dict[str, str]]) -> str:
        lines = [f"  {r.get('title','?')[:60]:60s} score={r.get('score','0'):>6s}  domain={r.get('domain','?')}" for r in records]
        return "\n".join(lines)
